...
public:
    ...
    mat4 Drag(float x, float y) {
      ...
      // cross = anchor_pos_ x current_pos
      vec3 rotation_axis = cross(anchor_pos_, current_pos);

      rotation = rotate(rotation, length(rotation_axis), rotation_axis);
...
private:
    ...
    void ProjectOntoSurface(vec3& p) const {
        ...
        float sumSquare = xSquare + ySquare;
        float radiusSquare = radius_ * radius_;

        bool isInsideCircle = sumSquare < (radiusSquare / 4.f);

        p.z = isInsideCircle
                ? sqrtf(radiusSquare - sumSquare)
                : (radiusSquare / 2.f) / sqrtf(sumSquare);
    }
